...
class MethodsGenerator:
    ...
    def provide_methods(self, file):
        for method in self.methods:
            ...
            if 'arguments' in method_props:
                for i, arg in enumerate(method_props['arguments']):
                    self.provide_metadata_for_write_method(file, i, method, arg['useParser'].format(arg['name']))
                ...
                self.provide_strategy_call_for_order(file, i, strategy_method_name)
                if method_props['returnType'] != RETURN_TYPES["VOID"]:
                    file.write("\t\treturn ")
                    file.write(method_props["useParser"].format("res.content") + "\n")
